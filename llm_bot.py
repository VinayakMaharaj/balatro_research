...
# Haiku for development - switch to claude-sonnet-4-6 for final paper runs
...
class LLMBot(BaseBot):
    ...
    Second tier in the three-tier comparison: FlushBot -> LLMBot -> RLBot.
    Pure zero-shot reasoning, no RAG, no fine-tuning.
    """
    ...
    def __init__(self, api_key: str | None = None, model: str = MODEL, **kwargs):
        ...
        if not key:
            raise ValueError(
                ...
                "Set $env:ANTHROPIC_API_KEY='your_key' or pass --api-key"
            )
        # Re-init balatro client since super().__init__ already set self.client
            ...
            if action == "buy_card":
                ...
                if idx < len(packs):
                    ...
                    if cost <= money:
                        ...
                        logger.warning(f"Cannot afford pack {idx}")
                ...
                if money >= reroll_cost:
                    ...
                    logger.warning("Cannot afford reroll")
